  ...
  try {
    const settings: Settings = {
      globalSettings: {
        ...
        enableInfoLogging: (document.getElementById(
          "enableInfoLogging"
      ...
      .each((index: number, element: HTMLElement) => {
        ...
        if (accountId) {
          settings.accountSettings[accountId] = {
            bArchiveUnread: (getElementForAccount(
              accountId,
              "archiveUnread"
            ) as HTMLInputElement).checked,
            daysUnread: Number(
              (getElementForAccount(
                accountId,
                "archiveUnreadDays"
              ) as HTMLInputElement).value
            ),
            bArchiveMarked: (getElementForAccount(
              accountId,
              "archiveStarred"
            ) as HTMLInputElement).checked,
            daysMarked: Number(
            ...
            bArchiveTagged: (getElementForAccount(
              accountId,
              "archiveTagged"
            ) as HTMLInputElement).checked,
            daysTagged: Number(
            ...
            bArchiveOther: (getElementForAccount(
              accountId,
              "archiveMessages"
            ) as HTMLInputElement).checked,
            daysOther: Number(
...
async function restoreOptions(): Promise<void> {
  ...
  (document.getElementById("enableInfoLogging") as HTMLInputElement).checked =
    settings.globalSettings.enableInfoLogging;
  document
    .querySelectorAll<HTMLInputElement>('input[name="archiveType"]')
    .forEach((element) => {
  ...
  for (const accountId in settings.accountSettings) {
    if (settings.accountSettings.hasOwnProperty(accountId)) {
      accountsSorted.push({
        account: AccountInfoProvider.findAccountInfo(
          accounts,
          accountId
        ) as AccountInfo,
  ...
  accountsSorted.forEach((accountInfos) => {
    ...
    getJQueryElementForAccount(accountId, "accountContent").data(
      "accountId",
      accountId
    );
    ...
    (getElementForAccount(
      accountId,
      "archiveUnread"
    ) as HTMLInputElement).checked = accountSetting.bArchiveUnread;
    (getElementForAccount(
      accountId,
      "archiveUnreadDays"
    ) as HTMLInputElement).value = accountSetting.daysUnread.toString();
    (getElementForAccount(
      accountId,
      "archiveStarred"
    ) as HTMLInputElement).checked = accountSetting.bArchiveMarked;
    (getElementForAccount(
      accountId,
      "archiveStarredDays"
    ) as HTMLInputElement).value = accountSetting.daysMarked.toString();
    (getElementForAccount(
      accountId,
      "archiveTagged"
    ) as HTMLInputElement).checked = accountSetting.bArchiveTagged;
    (getElementForAccount(
      accountId,
      "archiveTaggedDays"
    ) as HTMLInputElement).value = accountSetting.daysTagged.toString();
    (getElementForAccount(
      accountId,
      "archiveMessages"
    ) as HTMLInputElement).checked = accountSetting.bArchiveOther;
    (getElementForAccount(
      accountId,
      "archiveMessagesDays"
    ) as HTMLInputElement).value = accountSetting.daysOther.toString();
  });
}

function getJQueryElementForAccount(
  accountId: string,
  elementId: string
): JQuery {
