...
export class DefaultSettings {
  ...
  public convertPartialSettings(partialSettings: {
    [key: string]: unknown;
  }): Settings {
    const defaultSettings: Settings = this.getDefaultSettings();
    const concatedSettings: Settings = this.deepMerge(
      defaultSettings,
      partialSettings
    );
  ...
  private deepMerge<T extends Record<string, any>>(
    defaultValues: T,
    valuesToMerge: Record<string, any>
  ): T {
    ...
    for (const key in valuesToMerge) {
      if (valuesToMerge.hasOwnProperty(key)) {
        const elem: Record<string, any> = valuesToMerge[key] as Record<
          string,
          any
        >;
