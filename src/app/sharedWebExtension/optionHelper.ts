...
export class OptionHelper {
  public async loadCurrentSettings(): Promise<Settings> {
    ...
    try {
      ...
      this.ensureEveryExistingAccountHaveSettings(
        accounts,
        settings,
        oHandling
      );
  ...
  private removeOutdatedAccountsFromSettings(
    settings: Settings,
  ...
  private ensureEveryExistingAccountHaveSettings(
    accounts: AccountInfo[],
    settings: Settings,
    oHandling: DefaultSettings
  ): void {
    accounts.forEach((account) => {
      ...
      if (accountSetting === undefined) {
        settings.accountSettings[
          account.accountId
        ] = oHandling.getDefaultAccountSettings();
      }
    });
  }
