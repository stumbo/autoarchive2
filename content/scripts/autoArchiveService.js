...
let autoArchiveService = {
    ...
    copyListener: function(group) { // this listener is for Copy/Delete/Move actions
        this.QueryInterface = ChromeUtils.generateQI ? ChromeUtils.generateQI([Ci.nsIMsgCopyServiceListener, Ci.nsIMsgFolderListener]) : XPCOMUtils.generateQI([Ci.nsIMsgCopyServiceListener, Ci.nsIMsgFolderListener]);
    ...
    getFoldersFromWait4Folders: function() {
        ...
        Object.keys(self.wait4Folders).forEach( function(uri) {
            let folder;
            try {
                folder = MailUtils.getExistingFolder(uri);
            } catch(err) { autoArchiveLog.logException(err); }
            if ( folder ) {
    ...
    searchListener: function(rule, srcFolder, destFolder) {
        ...
        this.QueryInterface = ChromeUtils.generateQI ? ChromeUtils.generateQI([Ci.nsIFolderListener, Ci.nsIMsgSearchNotify]) : XPCOMUtils.generateQI([Ci.nsIFolderListener, Ci.nsIMsgSearchNotify]);
        this.messages = [];
        this.missingFolders = {};
        this.messagesDest = {};
        let allTags = {};
        ...
        this.onSearchHit = function(msgHdr, folder) {
            ...
            if ( rule.action == 'archive' ) {
                ...
                if ( !MailUtils.getIdentityForHeader(msgHdr).archiveEnabled ) return skipReason.cantArchive++;
            }
            ...
            if ( ["copy", "move"].indexOf(rule.action) >= 0 ) {
                ...
                let realDestFolder = MailUtils.getExistingFolder(realDest);
        ...
        this.processHeaders = function() {
            try {
                if ( rule.action != 'archive' ) {
                    ...
                    batchMover.folderDisplay = mail3PaneWindow.gFolderDisplay;
                    batchMover.msgWindow = mail3PaneWindow.msgWindow;
                    batchMover.oncomplete = function() {
                        ...
                        try {
                            if ( !batchMover.awsome_auto_archive_getFolders ) {
                                for ( let key in batchMover._batches ) {
                                    ...
                                    let archiveFolder = MailUtils.getExistingFolder(archiveFolderURI, false);
    ...
    doCopyDeleteMoveOne: function(group) {
        ...
        let srcFolder = MailUtils.getExistingFolder(group.src), destFolder = null;
        let busy = ( ['move', 'delete'].indexOf(group.action) >= 0 ) ? srcFolder.locked : false;
        if ( group.action != 'delete' ) {
            destFolder = MailUtils.getExistingFolder(group.dest);
    ...
    _doMoveOrArchiveOne: function() {
        autoArchiveLog.info("Enter _doMoveOrArchiveOne");
        ...
        try {
            srcFolder = MailUtils.getExistingFolder(rule.src);
            if ( ["move", "copy"].indexOf(rule.action) >= 0 ) {
                destFolder = MailUtils.getExistingFolder(rule.dest);
